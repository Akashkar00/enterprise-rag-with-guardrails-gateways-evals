...
# Force standard asyncio event loop policy so nest_asyncio can patch it (fixes uvloop error)
...
with tab3:
    ...
    if not st.session_state.pipeline_done:
        ...
        st.markdown(
            ...
            "LLM-based metrics use `JUDGE_GROQ_API` key — samples are scored one at a time "
            "with 40s cooldowns between samples to stay within Groq's **6,000 TPM** on-demand limit. "
            "Total runtime: ~50 min."
        )
        st.info(
            "Token key used: `JUDGE_GROQ_API` (separate from production key). "
